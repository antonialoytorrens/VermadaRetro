...
ifeq ($(BUILD_TYPE), debug)
	...
	#CXXFLAGS += -fsanitize=float-divide-by-zero -fsanitize=float-cast-overflow
	#CXXFLAGS += -fno-omit-frame-pointer -fno-sanitize=null -fno-sanitize=alignment
...
ifeq ($(CC), gcc)
	CXXFLAGS += -Wall -Wempty-body -Werror -Wpedantic -Wstrict-prototypes -Werror=maybe-uninitialized -Warray-bounds -Wno-aggressive-loop-optimizations
else  # Clang
	CXXFLAGS += -Wall -Wempty-body -Wpedantic -Wstrict-prototypes -Werror=uninitialized -Warray-bounds
endif
...
# Set targets for debug and release builds
debug: BUILD_TYPE = debug
debug: $(PROG) $(MAP_PROG)

release: $(PROG) $(MAP_PROG)
